...
{
    /// <summary>
    /// classe de base permettant la gestion d'un objet de type texte.
    /// </summary>
    public class CSBF1TextObject
    {
        ...
        public CSBF1TextLayout Layout { get; set; }

        public CSBF1TextVisualStyle VisualStyle {get;set;}
        ...
        {
            this.DecomposeData(rawData,ref globalIndex);
        }
        ...
        private void DecomposeData(byte[] rawData,ref int globalIndex)
        {
            ...
            this.Text = new CSBF1Text(ref globalIndex, rawData);
        }
        ...
        public byte[] RecomposeRawData()
        {
            ...
            if (VisualStyle.PrimaryColor.Count != VisualStyle.SecondaryColor.Count)
                throw new InvalidOperationException("Les deux palettes SBF1 doivent avoir la même taille.");

            //visual Styles
            rawData.AddRange(CGeneric.ConvertIntToByteArray(this.VisualStyle.PrimaryColor.Count));

            for (int i = 0; i < this.VisualStyle.PrimaryColor.Count; i++)
            {
                rawData.Add(this.VisualStyle.PrimaryColor[i].R);
                rawData.Add(this.VisualStyle.PrimaryColor[i].G);
                rawData.Add(this.VisualStyle.PrimaryColor[i].B);
                rawData.Add(this.VisualStyle.PrimaryColor[i].A);
            }
            for (int i = 0; i < this.VisualStyle.SecondaryColor.Count; i++)
            {
                rawData.Add(this.VisualStyle.SecondaryColor[i].R);
                rawData.Add(this.VisualStyle.SecondaryColor[i].G);
                rawData.Add(this.VisualStyle.SecondaryColor[i].B);
                rawData.Add(this.VisualStyle.SecondaryColor[i].A);
            }
            ...
            for (int i = 0; i < this.Layout.XPositions.Count; i++)
                rawData.AddRange(CGeneric.ConvertIntToByteArray(this.Layout.XPositions[i]));

            //texte
            rawData.AddRange(CGeneric.ConvertIntToByteArray(this.Text.Character.Count));
            for (int i = 0; i < this.Text.Character.Count; i++)
            {
                rawData.Add((byte)(this.Text.Character[i].RawData >> 8));
                rawData.Add((byte)this.Text.Character[i].RawData);
            }
